...
async def get_tweet_like_by_tweet_id_and_user_id(
    ...
    stmt = select(TweetLike)\
        .filter(TweetLike.tweet_id == tweet_id, TweetLike.user_id == user_id)
    result = await session.execute(stmt)
    return result.scalars().first()
...
async def create_tweet(content: str, author: User, attachments: List, session: AsyncSession) -> Tweet:
