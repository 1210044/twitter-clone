...
async def get_tweets(
    api_key: str = Header(None), session=Depends(get_session)
) -> TweetsOut:
...
async def create_tweet(
    tweet: TweetIn, api_key: str = Header(...), session=Depends(get_session)
) -> TweetOut:
    ...
    tweet_instance = await TweetService.create_tweet(
...
async def delete_tweet_by_id(
    tweet_id: int, api_key: str = Header(...), session=Depends(get_session)
) -> StatusResponse:
...
async def delete_like_tweet_by_id(
    ...
    tweet_like = await TweetService.get_tweet_like_by_tweet_id_and_user_id(
        tweet_id, user.id
    )
    if tweet_like:
