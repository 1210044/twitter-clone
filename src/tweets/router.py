...
async def get_tweets(api_key: str = Header(None), session = Depends(get_session)) -> TweetsOut:
...
async def create_tweet(tweet: TweetIn, api_key: str = Header(...), session = Depends(get_session)) -> TweetOut:
